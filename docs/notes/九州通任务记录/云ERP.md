...
> 提交git：2025年12月-268345-42138-客户合同-发起合同优化

SQL脚本：
```sql
-- 268345-42138-客户合同-发起合同优化（合同表加入合同模板版本号，用于校验模板是否被修改）
ALTER TABLE `weberp`.`tb_ele_contract`
    ADD COLUMN `template_revision` int NULL DEFAULT 1 COMMENT '合同模板版本号' AFTER `signed_file_url`;
```

| 同步环境 | 代码                                 | nacos配置 | SQL脚本 |
|------|------------------------------------|---------|-------|
| dev  | <Badge type="tip" text="master" /> | 无       | 已同步   |
